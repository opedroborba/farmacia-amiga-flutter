...
class FAFarmaciaAmigaSupabaseUser extends BaseAuthUser {
  FAFarmaciaAmigaSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> fAFarmaciaAmigaSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = FAFarmaciaAmigaSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
